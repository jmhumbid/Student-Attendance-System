...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function startScanning() {
        ...
        if (!isClassActive()) {
            instructionText.textContent = 'Cannot start scanner. Class is not in session.';
            console.log('Class is not active, cannot start scanner.');
            return;
        }
        ...
        try {
            ...
            if(stopScanningBtn) stopScanningBtn.style.display = 'block';
            if(instructionText) instructionText.style.display = 'block';
            if(video) video.style.display = 'block';
            if(attendanceResult) attendanceResult.textContent = '';
            ...
            errorDiv.textContent = 'Cannot access camera: ' + err.message;
            document.querySelector('.container').insertBefore(errorDiv, document.querySelector('.scanner-section'));
        }
    }
    ...
    function stopScanning() {
        ...
        if(stopScanningBtn) stopScanningBtn.style.display = 'none';
    ...
    function scanQRCode() {
        // Ensure necessary elements exist before proceeding with scan
        if (!scanning || (video && video.readyState < video.HAVE_ENOUGH_DATA) || !canvas || !ctx) {
             requestAnimationFrame(scanQRCode);
             return; // Ensure video, canvas, and context are ready
         }

        // Check if class is still active before scanning
        if (!isClassActive()) {
            stopScanning();
            return;
        }
    ...
    function recordAttendance(studentId) {
        if (scanning) {
            ...
            const classId = parseInt('<?= $selected_class_id ?? 'null' ?>', 10); // Parse class_id as integer

            // Ensure classId is a valid number before making AJAX call
            if (isNaN(classId)) {
                console.error('recordAttendance: Invalid classId:', '<?= $selected_class_id ?? 'null' ?>');
                attendanceResult.textContent = 'Error: Class not selected or invalid Class ID.';
                attendanceResult.className = 'attendance-result error';
                setTimeout(function() { // Resume scanning after a delay if still needed
                     if(isClassActive() && !stopScanningBtn.classList.contains('hidden')) {
                         scanning = true;
                         requestAnimationFrame(scanQRCode);
                     }
                 }, 2000);
    ...
    // Add a check to ensure buttons exist before adding event listeners
    if(startCameraBtn) {
    ...
    // Initial state check - only run if scanner elements exist
    if(startCameraBtn) {
       updateScannerState();
        // Check every minute if class is still active
