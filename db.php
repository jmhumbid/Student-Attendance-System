...
function add_instructor($full_name, $instructor_id, $department, $email, $username) {
    global $conn;
    $status = 'unverified';
    $verification_token = bin2hex(random_bytes(32));
    $stmt = $conn->prepare('INSERT IGNORE INTO instructors (full_name, instructor_id, department, email, username, status, verification_token) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->bind_param('sssssss', $full_name, $instructor_id, $department, $email, $username, $status, $verification_token);
    if ($stmt->execute()) {
...
function verify_instructor_account($token, $password) {
    global $conn;
    $encoded_password = base64_encode($password);
    $stmt = $conn->prepare('UPDATE instructors SET password = ?, status = "verified", verification_token = NULL WHERE verification_token = ? AND status = "unverified"');
    $stmt->bind_param('ss', $encoded_password, $token);
    $stmt->execute();
...
function get_instructor_by_email($email) {
    global $conn;
    $stmt = $conn->prepare('SELECT id, username, password, full_name, email, failed_login_attempts, lockout_until, last_login, suspended FROM instructors WHERE email = ?');
...
function get_all_instructors() {
    global $conn;
    $res = $conn->query('SELECT id, full_name, instructor_id, department, email, username, status, suspended, failed_login_attempts, lockout_until, last_login FROM instructors');
    $instructors = [];
    while ($row = $res->fetch_assoc()) {
        $instructors[] = $row;
    }
...
function suspend_instructor($instructor_id) {
    global $conn;
    $stmt = $conn->prepare('UPDATE instructors SET suspended = "yes" WHERE id = ?');
    $stmt->bind_param('i', $instructor_id);
    $result = $stmt->execute();
    $stmt->close();
    return $result;
...
function unsuspend_instructor($instructor_id) {
    global $conn;
    $stmt = $conn->prepare('UPDATE instructors SET suspended = "no" WHERE id = ?');
    $stmt->bind_param('i', $instructor_id);
    $result = $stmt->execute();
    $stmt->close();
    return $result;
...
function send_instructor_suspended_email($to_email, $full_name) {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Account Suspended - QR Lab System';
        $mail->Body = "<div style='font-family:Inter,sans-serif;background:#f8fafc;padding:32px;'>
            <div style='max-width:480px;margin:auto;background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08);padding:32px;text-align:center;'>
                <img src='https://cdn-icons-png.flaticon.com/512/3135/3135715.png' width='64' style='margin-bottom:16px;'>
                <h2 style='color:#b91c1c;margin-bottom:8px;'>Account Suspended</h2>
                <p style='color:#374151;font-size:16px;margin-bottom:24px;'>
                    Dear <b>" . htmlspecialchars($full_name) . "</b>,<br><br>
                    Your instructor account on <b>QR Lab System</b> has been <span style='color:#b91c1c;font-weight:bold;'>suspended</span> by the administrator.<br><br>
                    You will not be able to access your account until it is reactivated.<br><br>
                    If you believe this is a mistake, please contact the system administrator.
                </p>
                <div style='margin-top:32px;color:#6b7280;font-size:13px;'>
                    <hr style='margin-bottom:12px;'>
                    <p>This is an automated message. Please do not reply.<br>© " . date('Y') . " QR Lab System.</p>
                </div>
            </div>
        </div>";
        $mail->AltBody = "Your QR Lab System instructor account has been suspended. Please contact the administrator.";
        $mail->send();
        return true;
    } catch (Exception $e) {
...
function send_instructor_unsuspended_email($to_email, $full_name) {
    $mail = new PHPMailer(true);
    try {
        $mail->isSMTP();
        $mail->Host = 'smtp.gmail.com';
        ...
        $mail->addAddress($to_email, $full_name);
        $mail->isHTML(true);
        $mail->Subject = 'Account Reactivated - QR Lab System';
        $mail->Body = "<div style='font-family:Inter,sans-serif;background:#f8fafc;padding:32px;'>
            <div style='max-width:480px;margin:auto;background:#fff;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08);padding:32px;text-align:center;'>
                <img src='https://cdn-icons-png.flaticon.com/512/3135/3135715.png' width='64' style='margin-bottom:16px;'>
                <h2 style='color:#059669;margin-bottom:8px;'>Account Reactivated</h2>
                <p style='color:#374151;font-size:16px;margin-bottom:24px;'>
                    Dear <b>" . htmlspecialchars($full_name) . "</b>,<br><br>
                    Your instructor account on <b>QR Lab System</b> has been <span style='color:#059669;font-weight:bold;'>re-activated</span> by the administrator.<br><br>
                    You may now log in and continue using the system.<br><br>
        ...
        $mail->AltBody = "Your QR Lab System instructor account has been re-activated. You may now log in.";
        $mail->send();
        return true;
    } catch (Exception $e) {
        error_log('Unsuspension email error: ' . $e->getMessage());
        return false;
    }
}
?>