...
<body class="bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 min-h-screen flex">
    ...
    <div class="main-content flex-1 p-8 flex flex-col gap-8">
        ...
        <?php if ($class): ?>
            ...
            <div class="glass-card bg-white/80 rounded-2xl shadow-lg p-8 mb-8">
                <h2 class="text-2xl font-bold text-gray-800 mb-4 flex items-center gap-2"><i class="fas fa-calendar-alt text-pink-500"></i>View Attendance by Date</h2>
                <div id="calendar" class="mt-4"></div>
                <div id="attendanceList" class="mt-6">
                    <p class="text-gray-600">Click on a date to view detailed attendance records.</p>
                </div>
            </div>
            ...
            if (calendarEl) {
                const calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    events: function(info, successCallback, failureCallback) {
                        const currentClassId = <?php echo json_encode($class_id); ?>;
                        
                        console.log('Fetching attendance for date range:', info.start, info.end);

                        // Pass start and end dates for range query
                        fetch(`get_attendance_by_date.php?class_id=${currentClassId}&start=${info.startStr}&end=${info.endStr}`)
                            .then(response => response.json())
                            .then(data => {
                                console.log('Raw data from backend (range query):', data);
                                if (data.success && data.attendance) {
                                    // Filter for present students with attendance time for calendar events
                                    const presentStudentsAttendance = data.attendance.filter(record => 
                                        record.status === 'Present' && record.attendance_time
                                    );

                                    const events = presentStudentsAttendance.map(record => {
                                        // Create a Date object from the attendance time string
                                        const attendanceDateTime = new Date(record.attendance_time);
                                        
                                        // Check if the Date object is valid before creating the event
                                        if (!isNaN(attendanceDateTime.getTime())) {
                                            return {
                                                title: `${record.name} (${record.student_id})`,
                                                start: attendanceDateTime, // Use the valid Date object
                                                className: 'present', // Only present students appear as events
                                                extendedProps: {
                                            ...
                                            console.warn('Invalid attendance time received for student:', record);
                                            return null; // Return null for invalid dates, these will be filtered out
                                        }
                                    }).filter(event => event !== null); // Filter out any null entries from invalid dates
                                    
                                    console.log('Processed events for calendar (range query):', events);
                                    successCallback(events);
                                } else {
                                    console.log('Backend returned no attendance data or success was false (range query).', data);
                                    successCallback([]); // Return empty array if no data or error
                                }
                            })
                            .catch(error => {
                                console.error('Error fetching attendance (range query):', error);
                                failureCallback(error); // Indicate error to FullCalendar
                            });
                    },
                    eventClick: function(info) {
                        ...
                        // Use the event start time directly for display
                        const time = event.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                        
                        // Show event details in the attendance list (for clicking on an event in day/week view)
                        const attendanceList = document.getElementById('attendanceList');
                        attendanceList.innerHTML = `
                            <div class="bg-white rounded-lg shadow p-4">
                                <h3 class="text-lg font-semibold mb-2">Attendance Details</h3>
                                <p><strong>Student:</strong> ${studentName}</p>
                                <p><strong>Status:</strong> <span class="${status === 'Present' ? 'text-green-600' : 'text-red-600'}">${status}</span></p>
                                <p><strong>Time:</strong> ${time}</p>
                            </div>
                        `;
                    },
                    dateClick: function(info) {
                        // Fetch and display all attendance records for the clicked date (for list below calendar)
                        const currentClassId = <?php echo json_encode($class_id); ?>;
                        const clickedDate = info.dateStr; // YYYY-MM-DD
                        
                        console.log('Date clicked (single date query):', clickedDate);

                        // Pass only the date for single date query
                        fetch(`get_attendance_by_date.php?class_id=${currentClassId}&date=${clickedDate}`)
                            .then(response => response.json())
                            .then(data => {
                                const attendanceList = document.getElementById('attendanceList');
                                console.log('Raw attendance data for clicked date (single date query):', data);
                                if (data.success && data.attendance && data.attendance.length > 0) {
                                    const studentListHtml = data.attendance.map(student => {
                                        ...
                                        console.log('Processing attendance record (single date query):', student);

                                        let timeDisplay = 'N/A'; // Default for absent or missing time
                                        if (student.status === 'Present' && student.attendance_time) {
                                            // Use the returned timestamp directly to create a Date object
                                            const attendanceDate = new Date(student.attendance_time);
                                            if (!isNaN(attendanceDate.getTime())) { // Check if the date is valid
                                                 timeDisplay = attendanceDate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                                            } else {
                                                // Fallback if Date object creation from timestamp string fails
                                                timeDisplay = student.attendance_time; // Display raw time string as fallback
                                            }
                                        }

                                        return `
                                            <div class="mb-2 p-2 bg-gray-50 rounded">
                                                <p class="text-gray-800">${student.name} (<span class="font-mono text-sm">${student.student_id}</span>)</p>
                                                <p class="text-sm">Time: ${timeDisplay} - <span class="font-semibold ${statusColor}">${student.status}</span></p>
                                            </div>
                                        `;
                                    }).join('');
                                    attendanceList.innerHTML = `
                                        <h3 class="text-lg font-semibold mb-4">Attendance for ${new Date(clickedDate).toLocaleDateString()}</h3>
                                        ${studentListHtml}
                                    `;
                                } else {
                            ...
                            .catch(error => {
                                console.error('Error fetching attendance (single date query):', error);
